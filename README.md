...
日程通常更新更近，且可能带有较高的重要度，因此旧版按单一总分截取时容易占满前几条；这不表示其它记忆无法检索。`1.7.0` 起，面板召回测试和主动召回工具会先按开放事项、自我时间线、用户事实、当前窗口、阶段总结和稳定记忆分槽，再补齐剩余名额。具体的时间或日程问题仍会优先返回相关日程，泛查询则会保留类型多样性。
